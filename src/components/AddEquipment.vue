...
async function validateInput (dataInput) {
  let isValid = true;
  ...
  const tittleArray = await checkTittleEquipment();
    ...
    if (!dataInput.tittle.value) {
      ...
      for (const tittle of tittleArray) {
        if (dataInput.tittle.value == tittle) {
          dataInput.tittle.error = 'Название уже существует';
          isValid = false;
          break
        } else {
          isValid = true;
...
async function postEquipment () {
  ...
  try {
    const response = await axios.post(`https://6712a5e16c5f5ced662443cb.mockapi.io/api/v2/inventories/`,
      updatedProduct.value
    );
...
const checkTittleEquipment = async () => {
  let tittleArray = [];
  if (!equipmentStore.equipmentList.value || equipmentStore.equipmentList.length === 0) {
    try {
    const allEquipment = await axios.get('https://6712a5e16c5f5ced662443cb.mockapi.io/api/v2/inventories');
    equipmentStore.setEquipment(allEquipment.data);
    } catch (error) {
      console.error(error);
    } finally {
      loadingStore.changeValueLoading(false);
  ...
  for (const element of equipmentStore.equipmentList) {
    tittleArray.push(element.title)
  }
  return tittleArray
};
...
watch(photosArray.value, (newValue) => {
  ...
  const invalidUrls = newValue.filter(photo => photo.trim() !== '' && !urlPattern.test(photo));
  const hasEmptyFields = newValue.some(photo => photo.trim() === '');
  
  if (invalidUrls.length > 0) {
    checkError.value = 'Убедитесь, что ссылки начинаются с http:// или https://';
...
<template>
  <main>
    <div class="edit-form-wrapper">
      <form @submit.prevent="postEquipment()">
        ...
        <div class="wrapper" v-for="(data, key) in dataInput" :key="key">
          ...
          <div v-if="key === 'description'"
            ...
            <div class="info-error">
              ...
              <h3
                :class="{'input-invalid' : data.value && data.value.length > 255 || data.value.length === 0}"
              >
        ...
        <div class="photo-wrapper" v-if="photosArray.length > 0">
          <h1>Фото</h1>
          <div class="field" 
            v-for="(photo, index) in photosArray" 
            :key="index" >
            <input type="text" 
              v-model="photosArray[index]"
              placeholder="Введите ссылку на изображение" 
            />
            <div class="remove-btn-wrapper" v-if="photosArray && photosArray.length > 1">
              <button type="button" @click="removeField(index)">
                <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 24 24"><g fill="none" stroke="rgba(255, 116, 97, 1)" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M7.5 12h9"/></g></svg>
            </button>
          </div>
        ...
        <div class="check-container" v-if="isCheckPhotos && photosArray && photosArray.length > 0">
          <h1>Проверка фото: </h1>
          <div class="photos-for-check" 
            v-for="(photo, index) in photosArray" 
            :key="index"
